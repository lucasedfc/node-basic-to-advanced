...
io.on('connection', (client) => {
  client.on('loginChat', (data, callback) => {
    ...
    client.broadcast.to(data.room).emit('listOfUsers', users.getUsersByRoom(data.room));
  ...
  client.on('disconnect', () => {
    ...
    client.broadcast.to(userDeleted.room).emit('listOfUsers', users.getUsersByRoom(userDeleted.room));
  });
